...
class Helo {
  ...
  async generate(dir) {
    ...
    if (boilerplate.package) {
      ...
      this.processFiles(templateDir, fullpath, boilerplate.name)
      return
    }
    this.errlog('bilerplate.package is not exist')
  }

  async processFiles(boilerplate, savePath, boilerplateName) {
    const { prompts } = await import(boilerplate + '/index.js')
    ...
    const questions = prompts.map((item) => ({
      ...
      type: 'input',
      default: item.key === 'name' ? this.destDirectory : '',
    }))
