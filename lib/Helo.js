...
class Helo {
  ...
  async listBoilerplates() {
    ...
    // const chooses = this.compose(boilerplates)
    // console.log(chooses)
    this.log(`
      List templates: \n
      ${boilerplates
  ...
  async husky() {
    try {
      await exec('git init', { cwd: this.destDirPath })
      this.log(
        '\n' +
      ...
      this.warnlog('Husky init failed due to ' + ex.toString())
      this.warnlog(
        'You can install Husky manually, see https://typicode.github.io/husky/#/?id=automatic-recommended '
      )
    }
  }
  ...
  compose(boilerplates) {
    return boilerplates.map((b) => ({
      name: `${b.name} (${b.description})`,
      value: b,
    }))
  }

  async pickBoilerplate(boilerplates) {
    const choices = this.compose(boilerplates)
